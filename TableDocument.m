...
//        theResult = [mySQLConnection queryString:@"SHOW VARIABLES LIKE \"version\""];
        ...
        [tableWindow setTitle:[NSString stringWithFormat:@"(MySQL %@) %@@%@/%@", mySQLVersion, [userField stringValue],
                                    [hostField stringValue], [databaseField stringValue]]];
		[variablesWindow setTitle:[NSString stringWithFormat:@"(MySQL %@) %@@%@/",
...
*/
{
    ...
    if ( [[hostField stringValue] isEqualToString:@""]  && [[socketField stringValue] isEqualToString:@""] ) {
        ...
        //connect to socket
            mySQLConnection = [[SMCPConnection alloc] initToSocket:[socketField stringValue]
                                    withLogin:[userField stringValue]
                                    password:[passwordField stringValue]];
            [hostField setStringValue:@"localhost"];
        } else {
        //connect to host
            mySQLConnection = [[SMCPConnection alloc] initToHost:[hostField stringValue]
                                    withLogin:[userField stringValue]
                                    password:[passwordField stringValue]
...
//        [favoritesButton selectItemWithTitle:selectedFavorite];
        ...
        NSString *name = [[[prefs objectForKey:@"favorites"] objectAtIndex:[favoritesButton indexOfSelectedItem]-3]
                            objectForKey:@"name"];
        NSString *host = [[[prefs objectForKey:@"favorites"] objectAtIndex:[favoritesButton indexOfSelectedItem]-3]
                            objectForKey:@"host"];
        NSString *socket = [[[prefs objectForKey:@"favorites"] objectAtIndex:[favoritesButton indexOfSelectedItem]-3]
                            objectForKey:@"socket"];
        NSString *user = [[[prefs objectForKey:@"favorites"] objectAtIndex:[favoritesButton indexOfSelectedItem]-3]
                            objectForKey:@"user"];
        NSString *port = [[[prefs objectForKey:@"favorites"] objectAtIndex:[favoritesButton indexOfSelectedItem]-3]
                            objectForKey:@"port"];
        NSString *database = [[[prefs objectForKey:@"favorites"] objectAtIndex:[favoritesButton indexOfSelectedItem]-3]
        ...
        [passwordField setStringValue:[keyChainInstance
                        getPasswordForName:[NSString stringWithFormat:@"Shift : %@", name]
...
//write favorites and password
    NSDictionary *newFavorite = [NSDictionary
                        dictionaryWithObjects:[NSArray arrayWithObjects:favoriteName, host, socket, user, port, database, nil]
    ...
    if ( ![password isEqualToString:@""] )
        [keyChainInstance addPassword:password forName:[NSString stringWithFormat:@"Shift: %@", favoriteName]
                account:[NSString stringWithFormat:@"%@@%@/%@", user, host, database]];
...
-(IBAction)toggleConsole:(id)sender
{
	if ([self consoleIsOpened])
		[sender setTitle:@"Show Console"];
	else
		[sender setTitle:@"Hide Console"];
...
// set encoding of connection and client
	...
	if ( [[mySQLConnection getLastErrorMessage] isEqualToString:@""] ) {
		[mySQLConnection setEncoding:[SMCPConnection encodingForMySQLEncoding:[encoding UTF8String]]];
	} else {
		[self detectEncoding];
	}
...
*/
{
	...
	[mySQLConnection setEncoding:[SMCPConnection encodingForMySQLEncoding:[mysqlEncoding cString]]];	
...
- (IBAction)analyzeTable:(id)sender{
	[self tableOperation:@"ANALYZE"];
}

- (IBAction)checkTable:(id)sender{
	[self tableOperation:@"CHECK"];
}

- (IBAction)flushTable:(id)sender{
	[self tableOperation:@"FLUSH"];
}

- (IBAction)optimizeTable:(id)sender{
	[self tableOperation:@"OPTIMIZE"];
}

- (IBAction)repairTable:(id)sender{
	[self tableOperation:@"REPAIR"];
}
...
- (void)tableOperation:(NSString *)operation{
	...
	query = [NSString stringWithFormat:@"%@ TABLE `%@`",operation,[self table]];
...
*/
{
	if ([variablesWindow isVisible]){
		[sender setTitle:@"Show Variables"];
		[variablesWindow close];
		return;
	}
	
	[sender setTitle:@"Hide Variables"];
	...
	//[scrollView verticalScroller] //scroller needs to match the HUD display. why doesn't cocoa already take care of this?
...
*/
{
    ...
    SMCPResult *result = [mySQLConnection queryString:[NSString stringWithFormat:@"SHOW CREATE TABLE `%@`",
                                                            [self table]]];
	...
	[createTableSyntaxSheet setTitle:[self table]];
    [createTableSyntaxSheet makeKeyAndOrderFront:self];
}
...
*/
{
	...
	[toolbarItem setToolTip:NSLocalizedString(@"Show or hide the console which shows all MySQL commands performed by Shift", @"tooltip for toolbar item for show/hide console")];
	[toolbarItem setLabel:NSLocalizedString(@"Console", @"toolbar item for hide console")];
	[toolbarItem setImage:[NSImage imageNamed:@"Terminal.tiff"]];
...
*/
{
    return [NSArray arrayWithObjects:@"ToggleConsoleIdentifier", NSToolbarCustomizeToolbarItemIdentifier, NSToolbarFlexibleSpaceItemIdentifier, NSToolbarSpaceItemIdentifier, NSToolbarSeparatorItemIdentifier, nil];
}
...
*/
{
    return [NSArray arrayWithObjects:NSToolbarFlexibleSpaceItemIdentifier,@"ToggleConsoleIdentifier", nil];
}
