...
//error if file cannot be written
        if ( ![testString writeToFile:[sheet filename] atomically:YES encoding:NSUTF8StringEncoding error:errorStr] ) {
            NSBeginAlertSheet(NSLocalizedString(@"Error", @"error"), NSLocalizedString(@"OK", @"OK button"), nil, nil, tableWindow, self, nil, nil, nil,
	...
	// mac os 10.4 or later
		success = [fileContent writeToFile:[sheet filename] atomically:YES encoding:[SMCPConnection encodingForMySQLEncoding:[[tableDocumentInstance getSelectedEncoding] UTF8String]]  error:errorStr];
	} else {
	// mac os pre 10.4
...
//load file into string
	...
	// mac os 10.4 or later
		dumpFile = [NSString stringWithContentsOfFile:[sheet filename] encoding:[SMCPConnection encodingForMySQLEncoding:[[tableDocumentInstance getSelectedEncoding] UTF8String]] error:errorStr];
	} else {
	// mac os pre 10.4
...
//    [dump appendString:[NSString stringWithFormat:@"# Tables dumped %@\n# Created by CocoaMySQL (Copyright (c) 2002-2003 Lorenz Textor)\n#\n# Host: %@   Database: %@\n# ******************************\n\n", [NSDate date], [tableDocumentInstance host], [tableDocumentInstance database]]];
    [dump appendString:@"# Shift dump\n"];
    [dump appendString:[NSString stringWithFormat:@"# Version %@\n",
                            [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleVersion"]]];
    [dump appendString:@"# http://www.shiftosx.com\n#\n"];
...
//add header
        ...
        [string appendString:@"<!--\n-\n"];
        [string appendString:@"- Shift dump\n"];
        [string appendString:[NSString stringWithFormat:@"- Version %@\n",
                            [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleVersion"]]];
        [string appendString:@"- http://www.shiftosx.com\n-\n"];
